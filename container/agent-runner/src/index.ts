...
async function main(): Promise<void> {
  ...
  const sdkEnv: Record<string, string | undefined> = {
    ...process.env,
    // Auto-compact: for NW workers, the shim caps max_tokens to prevent
    // context overflow. The SDK's default auto-compact (based on its internal
    // token estimate and the model's context window) handles compaction.
    // We don't override the threshold here — the SDK's tokenizer and the
    // NW API's tokenizer diverge significantly, making percentage-based
