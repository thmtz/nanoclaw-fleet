...
src/container-runner.ts               container/agent-runner/
    ...
    │                                      │
    ├── data/v2-sessions/{group}/{session}/ ──> /workspace (per-session)
    ├── groups/{folder} ───────────> /workspace/agent
    ├── data/v2-sessions/{group}/.claude-shared/ ──> /home/node/.claude/ (per-group)
    └── (mount-allowlist) extra dirs ──> /workspace/extra/*
```
...
echo '{"prompt":"What is 2+2?","groupFolder":"test","chatJid":"test@g.us","isMain":false}' | \
  docker run -i \
  -v $(pwd)/data/v2-sessions/{group}/{session}:/workspace \
