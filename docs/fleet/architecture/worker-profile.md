...
| `tools`       | Shell commands run during init (e.g. `uv tool install /workspace/your-tool --force`). Idempotent.                                                                                                                                                                                        |
...
3. **Repos.** For each `fleetProfile.repos` entry, `git clone` into `/workspace/<repo-name>/` (the per-session workspace root) if it doesn't already exist. If `postClone` is set, run it from the cloned directory.
